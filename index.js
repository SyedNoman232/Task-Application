// data will store in the state

const state = {
  taskList: [],
};

//  selection  document object models
const taskContents = document.querySelector(".task__contents");
const taskModal = document.querySelector(".task__modal__body");

const htmlTaskContent = ({ id, title, description, type, url }) => `
    <div class="col-md-6 col-lg-4 mt-3 " id=${id} key=${id}>
        <div class="card shadow-sm task__card " style="background-color:#F2F4F4">
            ...
            <div class="card-body" style="background-color:#D1F2EB;">
            ${
              url
                ? `<img width="100%" src=${url} alt="card image cap" class="card-image-top md-3 rounded"/>`
            ...
            <div class="card-footer" style="background-color:#F2F4F4">
                <button type="button" style="background-color:#D1F2EB" class="btn btn-outline-primary float-right" data-bs-toggle="modal" data-bs-target="#showTask" id=${id} onclick='openTask.apply(this, arguments)'>
                    Open Task
                </button>
            </div>
        </div>
...
// when user add the info it will store in th our array  and also display
// using data oprator
const handleSubmit = () => {
  const id = `${Date.now()}`;
  const input = {
